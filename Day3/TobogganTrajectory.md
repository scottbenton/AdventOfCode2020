...
After creating a function to read in the board, the next step was to create a function that given a board, starting coordinates, and velocities, would return the number of trees on the given path. Since the board loops, I created this function using modulo in the x direction to ensure that we are always checking positions that exist on the board, and if that position is a tree, we add 1 to the count that gets returned.

For the solution to part 1, I just called that function with the parameters given.
